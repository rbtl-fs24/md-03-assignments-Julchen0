...
1.  Look at the plot that you created. What do you notice about the order of the bars / order of the legend?
    ...
    legend order is the same order as the bars

2.  What would you want to change?

    maybe move the bars with the least amount of "value" to the top
...
3.  Why did we remove "safely managed sanitation services" from the data set in Task 3?
    ...
    2: Removed 6 rows containing missing values (`position_stack()`). 
    ```

    these rows contain no value (NA) so they get removed since they can't be plottet
...
2.  What do you observe when you look at the code and plot?
    ...
    the width of the bin is not defined which makes problems, values get removed
